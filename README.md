...
To enable Supabase Row Level Security without breaking this project:

- use the anon key in the Streamlit app for read-only access to `pitchers`
- use the service-role key for ingest and retraining scripts that read or write `pitches`, `pitchers`, and `pipeline_state`
- keep RLS enabled on all three tables
...
create policy "public can read pitchers"
on public.pitchers
for select
...
The app does not authenticate end users today, so `pitchers` needs a `select` policy for `anon` if you want the manual predictor to keep loading pitcher names. The backend scripts should use the service-role key, which bypasses RLS and does not need table policies for normal operation.
